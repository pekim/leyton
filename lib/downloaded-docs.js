...
function getVersion(version, callback) {
  ...
  function readFiles(callback) {
    ...
    fs.readdir(versionDir, function filesInVersionDir(err, files) {
      if (err) {
        callback(err);
      } else {
        async.forEach(files, read,
          function(err) {
            callback(err, filesContents);
          });
      }
    });

    function read(file, callback) {
      fs.readFile(path.join(versionDir, file), 'utf8', function fileContents(err, data) {
        if (err) {
          callback(err);
        } else {
          var name = file;
