...
var GithubDownloads = require('./downloads');
...
function gitCheckoutMaster(callback) {
  var git = spawn('git', ['checkout', 'master'], {cwd: repoDir});
  ...
  git.on('exit', function gitExit(code) {
    if (code !== 0) {
      callback('git checkout failed: exit code ' + code);
    } else {
      callback();
    }
  });
...
exports.getFiles = getUploadedFiles;

update(function(err, versionTags){
  if (err) {
